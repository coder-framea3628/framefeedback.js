...
.ff-logo img { width:100%; height:100%; object-fit:cover; display:block; border-radius:10px; }
...
.ff-privacy input[type="checkbox"] { accent-color: var(--accent) !important; }
...
/* hide maybe/secondary button visually (keeps structure in DOM) */
#ffLater { display: none !important; }
...
/* glow for excellent */
@keyframes ff-glow {
  0% { box-shadow: 0 0 8px rgba(211,173,131,0.2); transform: translateY(0); }
  50% { box-shadow: 0 0 32px rgba(211,173,131,0.35); transform: translateY(-2px); }
  100% { box-shadow: 0 0 8px rgba(211,173,131,0.2); transform: translateY(0); }
}
.ff-glow { animation: ff-glow 1.6s ease-in-out infinite; }
...
@media (max-width: 520px) {
  .ff-card { padding:12px; width:100%; max-width:none; border-radius:8px; height:auto; margin:0 6px; }
...
`;
    ...
    <div class="ff-card" role="dialog" aria-modal="true" aria-label="Avaliação da Frame" id="ffCard">
      <div class="ff-top">
        <div class="ff-top-left">
          <div class="ff-logo" aria-hidden="true">
            <img src="https://framerusercontent.com/images/nqe8sytY941OUcgvF17Y9qLajUc.png" alt="F" />
          </div>
          <div>
  ...
  overlay.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      // do not close panel, just hint user (per requirement)
      showToast('Para fechar, basta fechar seu navegador.');
    }
  });
  ...
  function closePanel() {
    // keep semantics but do not remove the card immediately if user requested that behavior;
    // we'll keep original functionality available via API but when user clicks close we only show a message.
  ...
  // change behaviour: clicking "Fechar" should show instruction instead of instantly removing (per request)
  ffClose.addEventListener('click', () => {
    showToast('Para fechar, basta fechar seu navegador.');
  });

  // keep ffLater logic but it's hidden via CSS; keep its handler to preserve structure
  ffLater.addEventListener('click', () => {
    showToast('Tudo bem! A gente volta depois :)');
    // still keep original close behavior behind the scenes if needed
  ...
  ratingItems.forEach(item => {
    ...
    function select() {
      ...
      // show "low" for <= 3; show "high" for >=4 (Bom e Excelente)
      if (val <= 3) reasonsLow.classList.add('active');
      if (val >= 4) reasonsHigh.classList.add('active');
      // glow effect only for Excelente (5)
      if (val === 5) {
        ffCard.classList.add('ff-glow');
      } else {
        ffCard.classList.remove('ff-glow');
      }
  ...
  ffToggleReport.addEventListener('click', () => {
    if (ffReport.style.display === 'none' || !ffReport.style.display) {
      ...
      // hide rating emojis (per requirement)
      if (ffRating) ffRating.style.display = 'none';
      // make submit reflect reporting
      ffSubmit.textContent = 'Enviar denúncia';
      // visually collapse reasons
      reasonsLow.classList.remove('active');
      reasonsHigh.classList.remove('active');
      // clear selected rating to avoid confusion
      ratingItems.forEach(it => it.classList.remove('ff-selected'));
      selectedRating = null;
    } else {
      // exit report mode
      ffReport.style.display = 'none';
      ffToggleReport.textContent = 'Denunciar atendente';
      if (ffRating) ffRating.style.display = '';
      ffSubmit.textContent = 'Enviar avaliação';
    }
  });

  /* submit simulation (no backend) */
  ffSubmit.addEventListener('click', () => {
    // If in report mode: require report fields and privacy but rating not required
    const inReportMode = (ffReport.style.display === 'flex' || ffReport.style.display === 'inline-flex');

    if (!inReportMode && !selectedRating) {
    ...
    // report-specific validation: require name + description
    if (inReportMode) {
      if (!ffReportName.value || !ffReportName.value.trim()) {
        showToast('Por favor, informe o nome do atendente que deseja reportar.');
        return;
      }
      if (!ffReportDesc.value || !ffReportDesc.value.trim()) {
        showToast('Por favor, descreva o motivo da denúncia.');
        return;
      }
    }
    ...
    setTimeout(() => {
      ...
      ffCard.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;gap:12px;padding:18px">
          ...
          <div style="margin-top:12px;width:100%;display:flex;gap:10px;justify-content:center">
            ...
            <a class="ff-btn ghost" href="https://frameag.com/contato" id="ffNewLink" target="_blank" rel="noopener">Central de ajuda</a>
          </div>
        </div>
      `;
      // small toast
      ...
      // hook buttons
      try {
        document.getElementById('ffCloseFinal').addEventListener('click', () => {
          // per behavior: show message rather than closing silently
          showToast('Para fechar, basta fechar seu navegador.');
        });
      } catch (e) {}
      // ffNewLink is an anchor to contact page - already in DOM
    }, 4200);
  });
